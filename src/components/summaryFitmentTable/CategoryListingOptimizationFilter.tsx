...
const CategoryListingOptimizationFilter: React.FC<CategoryFilterProps> = ({
  filter,
  setFilter,
  firstAllOptions,
  secondAllOptions,
  ...
  const fetchSummaryFitment = async ({
    ...
    try {
      ...
      dispatch(setListingCategoryScore(data.categoryFitmentScore));
  ...
  const handleCategoryChange = (
    ...
    const paramParts = [];
    const filterKeys = Object.keys(updatedFilter) as Array<
  ...
  return (
    <>
      ...
      {filterLevels.slice(1).map(({ level, options }, index) => {
        if (options.length === 0) return null; // Skip if options is empty
        return (
          <div key={index}>
            <FilterDropdowns
              label="Category"
...
export default CategoryListingOptimizationFilter;
